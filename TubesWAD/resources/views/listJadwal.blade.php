            ...
            <tbody>
                @foreach($konsuls as $konsul)
                    <tr>
                        ...
                        <td>
                            @if(!$konsul->status) <!-- Edit hanya jika status on going -->
                                <a href="{{ route('janji_temu.edit', $konsul->id) }}" class="btn btn-warning">Edit</a>
                                @else
                                <a href="{{ route('janji_temu.edit', $konsul->id) }}" class="btn btn-warning disabled" >Edit</a>
                            @endif
                        <td>
                            @if($konsul->status)
                        ...
                        <td>
                            <form action="{{ route('janji_temu.destroy', $konsul->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                @if($konsul->status) <!-- Hapus hanya jika status done -->
                                    <button type="submit" class="btn btn-danger">Delete</button>
                                @else
                                    <button type="submit" class="btn btn-danger disabled">Delete</button> <!-- Tombol Hapus dinonaktifkan -->
                                @endif
                            </form>
                        </td>
                    </tr>
